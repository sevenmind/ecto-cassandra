defmodule EctoCassandra.Integration.TypesTest do
  ...
  test "primitive types" do
    ...
    assert [1] =
             TestRepo.all(
               from(p in Post, where: p.visits == 1, select: p.visits, lock: "ALLOW FILTERING")
             )
    ...
    assert [true] =
             TestRepo.all(
               from(p in Post, where: p.public == true, select: p.public, lock: "ALLOW FILTERING")
             )

    # UUID
    assert [^uuid] =
             TestRepo.all(
               from(p in Post, where: p.uuid == ^uuid, select: p.uuid, lock: "ALLOW FILTERING")
             )
  ...
  test "uuid types" do
    assert %Post{} =
             post =
             TestRepo.insert!(%Post{title: "bid test", uuid: UUID.uuid4(), timeuuid: UUID.uuid1()})
