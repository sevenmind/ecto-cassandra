defmodule EctoCassandra do
  ...
  def create_keyspace(options) do
    keyspace =
      Keyword.fetch!(options, :keyspace) || raise ":keyspace is nil in repository configuration"
  ...
  defp primitive({_, _, _, _, _, _} = ip),
    do: ip |> Tuple.to_list() |> Enum.join(":") |> primitive
